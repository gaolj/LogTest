...
BOOST_LOG_INLINE_GLOBAL_LOGGER_DEFAULT(test_lg1, src::logger_mt)
...
static void thread_fun(boost::barrier& bar)
{
    ...
    {
        BOOST_LOG(test_lg1::get()) << "Log record " << i;
    }
}
...
BOOST_AUTO_TEST_CASE(bounded_async_log)
{
    try
    {
        ...
        shared_ptr< std::ostream > strm(new std::ofstream("logs/bounded_async_log.log"));
        if (!strm->good())
